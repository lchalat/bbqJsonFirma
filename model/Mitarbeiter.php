...
class Mitarbeiter implements Aenderbar, JsonSerializable {
    ...
    public static function insert($id) {
        ...
        $stmt = $pdo->prepare($sql);
        $vorgesetzter_id = is_object($id->getVorgesetzter()) ? $id->getVorgesetzter()->getId() : NULL;
        $stmt->execute([':vorname' => $id->getVorname(), ':nachname' => $id->getNachname(), ':geschlecht' => $id->getGeschlecht(), ':geburtsdatum' => $id->getGeburtsdatum(), ':abteilung_id' => $id->getAbteilung()->getId(), ':stundenlohn' => $id->getStundenlohn(), ':vorgesetzter_id' => $vorgesetzter_id]);
    }
